...
const STATUS: Record<TaskStatus, {
  ...
  PromptReady: { label: '提示词就绪', dotCls: 'bg-slate-500',  badgeCls: 'bg-slate-100 dark:bg-slate-800/70 text-slate-700 dark:text-slate-300', headerCls: 'text-slate-700 dark:text-slate-300' },
  Running:     { label: '执行中',    dotCls: 'bg-amber-500 animate-pulse',   badgeCls: 'bg-amber-50  dark:bg-amber-500/10  text-amber-700  dark:text-amber-400',  headerCls: 'text-amber-700  dark:text-amber-400'  },
  ...
  Scored:      { label: '已评分',    dotCls: 'bg-sky-500',     badgeCls: 'bg-sky-50    dark:bg-sky-500/10    text-sky-700    dark:text-sky-400',    headerCls: 'text-sky-700    dark:text-sky-400'    },
  Archived:    { label: '已归档',    dotCls: 'bg-stone-400',   badgeCls: 'bg-stone-100 dark:bg-stone-800/60  text-stone-500  dark:text-stone-400',  headerCls: 'text-stone-500  dark:text-stone-400'  },
};

const COLUMNS: TaskStatus[] = ['Claimed', 'PromptReady', 'Running', 'Submitted', 'Scored', 'Archived'];
...
export default function Board() {
  ...
  const handleCreateTask = async () => {
    ...
    try {
      ...
      addTaskToStore({
        id: created.id,
        projectId: String(created.gitlab_project_id),
              ...
              <div className="px-7 py-6 border-b border-stone-100 dark:border-stone-800 flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex items-center gap-2.5 mb-1.5">
                    ...
                    <span className={`text-xs font-bold uppercase tracking-wider ${STATUS[selected.status].badgeCls} px-2 py-0.5 rounded-full`}>
                ...
                {drawerLoading ? (
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-3 mb-8">
                      ...
                      <InfoCard label="创建时间" value={selected.createdAt} />
                      <InfoCard label="提示词" value={selectedTaskDetail?.prompt_text ? '已保存' : '未保存'} />
                    </div>
                    ...
                    <div className="rounded-2xl bg-stone-50 dark:bg-stone-800/50 border border-stone-200 dark:border-stone-700 px-4 py-4 mb-8 space-y-2">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="text-xs text-stone-400 dark:text-stone-500">
                          {selectedModelRuns.filter((run) => !isOriginModel(run.model_name)).length} 个模型副本
                        </span>
                      </div>
                      <p className="font-mono text-xs leading-6 text-stone-600 dark:text-stone-300 break-all">
                        {selectedTaskDetail?.local_path || '当前题卡未记录本地目录'}
                      </p>
                    </div>
                      ...
                      <div className="flex items-center justify-between gap-3 mb-3">
                        ...
                        <div className="flex items-center gap-2 text-[11px] font-semibold">
                          <span className="px-2 py-1 rounded-full bg-stone-100 dark:bg-stone-800 text-stone-500 dark:text-stone-400">
                            待处理 {selectedModelRuns.filter((run) => !isOriginModel(run.model_name) && run.status === 'pending').length}
                          </span>
                          <span className="px-2 py-1 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400">
                            执行中 {selectedModelRuns.filter((run) => !isOriginModel(run.model_name) && run.status === 'running').length}
                          </span>
                          <span className="px-2 py-1 rounded-full bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400">
                            已完成 {selectedModelRuns.filter((run) => !isOriginModel(run.model_name) && run.status === 'done').length}
                          </span>
                        </div>
                      </div>
                                ...
                                <div className="flex items-start justify-between gap-3">
                                  <div className="min-w-0">
                                    ...
                                    <div className="mt-2 space-y-1.5 text-xs text-stone-500 dark:text-stone-400">
                                      <p className="break-all">{run.local_path || '未记录副本目录'}</p>
                                      <p className="font-mono break-all">{run.branch_name || '尚未创建分支'}</p>
                                    </div>
                                  </div>
...
function TaskCard({
  ...
  return (
    ...
    >
      ...
      {status === 'Running' && task.totalModels > 0 && (
