...
export default function Settings() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [url, token, username, projectList, activeProject, accounts, providers] = await Promise.all([
  ...
  const handleSaveGitlab = useCallback(async () => {
    ...
    try {
      await Promise.all([
        setConfig('gitlab_url', gitlabUrl),
        setConfig('gitlab_token', gitlabToken),
        setConfig('gitlab_username', gitlabUsername),
      ]);
  ...
  const handleTestConnection = useCallback(async () => {
    ...
    try {
      const ok = await testGitLabConnection(gitlabUrl, gitlabToken);
  ...
  const openEditGithubModal = (account: GitHubAccountConfig) => {
    setGithubForm({
      ...
      token: account.token,
      defaultRepo: account.defaultRepo ?? '',
  ...
  const persistGithubAccounts = async (accounts: GitHubAccountConfig[]) => {
    const normalized = normalizeGitHubAccounts(accounts);
    await saveGitHubAccounts(normalized);
    setGithubAccounts(normalized);
  ...
  const handleSaveGithubAccount = async () => {
    ...
    const defaultRepo = githubForm.defaultRepo.trim();
    ...
    try {
      ...
      const nextAccounts = githubForm.id
        ? githubAccounts.map((account) => (account.id === githubForm.id ? nextAccount : account))
        : [...githubAccounts, nextAccount];

      const finalAccounts = nextAccount.isDefault
      ...
      await persistGithubAccounts(finalAccounts);
      closeGithubModal();
    } catch (error) {
      console.error(error);
  ...
  const handleDeleteGithubAccount = async (accountId: string) => {
    const nextAccounts = githubAccounts.filter((account) => account.id !== accountId);
    try {
      await persistGithubAccounts(nextAccounts);
    } catch (error) {
      console.error(error);
  ...
  const handleSetDefaultGithubAccount = async (accountId: string) => {
    const nextAccounts = githubAccounts.map((account) => ({
      ...account,
  ...
  const persistProviders = async (providers: LlmProviderConfig[]) => {
    const normalized = normalizeProviders(providers);
    await saveLlmProviders(normalized);
    setLlmProviders(normalized);
  ...
  const handleSaveProvider = async () => {
    ...
    try {
      ...
      const nextProviders = providerForm.id
        ? llmProviders.map((provider) => (provider.id === providerForm.id ? nextProvider : provider))
        : [...llmProviders, nextProvider];

      const finalProviders = nextProvider.isDefault
        ? nextProviders.map((provider) => ({
            ...provider,
            isDefault: provider.id === nextProvider.id,
          }))
        : nextProviders;

      await persistProviders(finalProviders);
      closeProviderModal();
    } catch (error) {
      console.error(error);
      ...
      <div className="flex-1 flex gap-6 min-h-0">
        ...
        <div className="flex-1 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-3xl overflow-y-auto">
          <div className="p-8">
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="pt-5 flex items-center justify-between border-t border-stone-100 dark:border-stone-800">
                      <div className="flex items-center gap-3">
                        <button onClick={handleTestConnection} disabled={testingConnection} className={btnSecondary}>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={handleSaveGitlab} disabled={savingGitlab} className={btnPrimary}>
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="min-w-0">
                              ...
                              <div className="mt-3 space-y-1 text-xs text-stone-500 dark:text-stone-400">
                                ...
                                <p>默认仓库: {account.defaultRepo?.trim() || '未设置'}</p>
                              </div>
                            </div>
            ...
            {activeTab === 'general' && (
              <section className="max-w-lg animate-in fade-in duration-150">
                <SectionHead title="通用设置" description="主题配置会立即生效，项目目录、模型、源码文件夹和源码仓库请使用左侧的新建项目入口管理" />
                ...
                {loading ? (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="rounded-2xl bg-stone-50 dark:bg-stone-800/50 border border-stone-200 dark:border-stone-700 px-4 py-3 text-sm text-stone-500 dark:text-stone-400">
                      项目名称、项目目录、模型列表、源码文件夹和源码仓库已迁移到侧边栏底部的“新建项目”入口。
                    </div>
                    <div className="rounded-2xl bg-stone-50 dark:bg-stone-800/50 border border-stone-200 dark:border-stone-700 px-4 py-4 space-y-2">
                      ...
                      <InfoText label="源码文件夹">{activeProject?.sourceModelFolder?.trim() || 'ORIGIN'}</InfoText>
                      <InfoText label="源码仓库">{activeProject?.defaultSubmitRepo?.trim() || '未设置'}</InfoText>
            ...
            {activeTab === 'data' && (
              <section className="max-w-lg animate-in fade-in duration-150">
                <SectionHead title="数据管理" description="导入、导出和清理任务数据" />
                <div className="space-y-3">
                  {[
                    { title: '导出数据', desc: '将所有任务和配置导出为 JSON', btn: '导出' },
                    { title: '导入数据', desc: '从之前导出的 JSON 文件恢复', btn: '导入' },
                  ].map((item) => (
                    <div
                      key={item.title}
                    ...
                    >
                      ...
                      <button className={btnSecondary}>{item.btn}</button>
                    </div>
                  ))}
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30">
                    <div>
                      ...
                      <p className="text-xs text-red-500/70 dark:text-red-400/60 mt-0.5">永久删除所有已归档的任务记录</p>
                    </div>
                    <button className="px-5 py-2.5 bg-white dark:bg-stone-900 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 border border-red-200 dark:border-red-900/40 rounded-full text-sm font-semibold transition-colors cursor-default">
                      清除
                    </button>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
          ...
          <div className="relative w-full max-w-lg rounded-3xl bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 shadow-2xl p-6">
            ...
            <div className="space-y-4">
              <Field label="账号名称" hint="例如：work-account / personal">
                <input
                  type="text"
              ...
              <Field label="默认仓库" hint="可选，提交中心会用它预填目标仓库">
                <input
                  type="text"
                  value={githubForm.defaultRepo}
                  onChange={(event) => setGithubForm((current) => ({ ...current, defaultRepo: event.target.value }))}
              ...
              <div className="pt-5 flex items-center justify-end gap-3 border-t border-stone-100 dark:border-stone-800">
                ...
                <button onClick={handleSaveGithubAccount} disabled={savingGithubAccount} className={btnPrimary}>
