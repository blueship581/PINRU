...
func (s *ChatService) ListSessions(taskID string) ([]store.ChatSession, error) {
	if strings.TrimSpace(taskID) == "" {
		return nil, fmt.Errorf("taskId 不能为空")
	}
	sessions, err := s.store.ListChatSessions(taskID)
	if err != nil {
		return nil, err
	}
	if sessions == nil {
...
func (s *ChatService) SendMessage(req SendMessageRequest) (*SendMessageResponse, error) {
	...
	// Start CLI
	cliResp, err := s.cliSvc.StartClaude(StartClaudeRequest{
		WorkDir:        req.WorkDir,
		Prompt:         fullPrompt,
		Model:          model,
	...
	if err != nil {
		if req.AutoSavePrompt {
			_ = s.store.FailTaskPromptGeneration(swm.Session.TaskID, "启动失败: "+err.Error(), promptStartedAt)
		}
		// Cleanup pre-allocated assistant message
		_ = s.store.DeleteChatSession("") // no-op, just don't leave dangling
		_ = s.store.UpdateChatMessage(assistantMsg.ID, "[启动失败: "+err.Error()+"]")
		return nil, err
	}
	...
	go func() {
		...
		offset := 0
		for {
			poll, err := s.cliSvc.PollOutput(PollOutputRequest{
			...
			if poll.Done {
				...
				if req.AutoSavePrompt {
					...
					} else if promptText, warning, err := s.persistGeneratedPrompt(swm.Session.TaskID, req.WorkDir, promptSnapshot, result, promptStartedAt); err != nil {
...
func capturePromptArtifactSnapshot(workDir string) promptArtifactSnapshot {
	info, err := os.Stat(promptArtifactPath(workDir))
	if err != nil {
...
func (s *ChatService) persistGeneratedPrompt(taskID, workDir string, snapshot promptArtifactSnapshot, cliOutput string, startedAt int64) (string, string, error) {
	...
	var warning string
	if !fromFile {
		if err := writePromptArtifact(workDir, promptText); err != nil {
