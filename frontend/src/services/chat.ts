...
export async function createSession(taskId: string, model: string): Promise<ChatSession> {
  return callService<ChatSession>('ChatService', 'CreateSession', { taskId, model });
}

export async function listSessions(taskId: string): Promise<ChatSession[]> {
  return callService<ChatSession[]>('ChatService', 'ListSessions', taskId);
}

export async function getSessionWithMessages(sessionId: string): Promise<SessionWithMessages> {
  return callService<SessionWithMessages>('ChatService', 'GetSessionWithMessages', sessionId);
}
...
export async function sendMessage(req: SendMessageRequest): Promise<SendMessageResponse> {
  return callService<SendMessageResponse>('ChatService', 'SendMessage', req);
}

export async function getMessage(messageId: string): Promise<ChatMessage> {
  return callService<ChatMessage>('ChatService', 'GetMessage', messageId);
}
